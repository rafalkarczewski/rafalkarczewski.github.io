...
Diffusion models are typically trained using a score matching objective, which seeks to approximate the score function $$\nabla \log p_t(\mathbf{x})$$ using a neural network $$ \mathbf{s}_\theta(\mathbf{x})$$. One common formulation is the denoising score matching objective <d-cite key="vincent2011connection"></d-cite>:
...
For a random variable $$X$$ with density $$p$$, the probability of $$X$$ beding in a set $$A$$ is given by the integral of the density over that region:
