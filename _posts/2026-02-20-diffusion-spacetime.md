...
To generate data, we reverse this process. There are two approaches<d-footnote>\(f_t = \frac{d \log \alpha_t}{dt}\) and \(g_t^2 = \sigma_t^2 \cdot -\frac{d \lambda_t}{dt}\), where \( \lambda_t = \log \frac{\alpha_t^2}{\sigma_t^2} \) is the log signal-to-noise ratio.</d-footnote>:
